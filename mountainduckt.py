...
f = open("mountainduck_settings_folder.txt", "r")
...
for info in bookmark_info:
	...
	if handler.query == "" or handler.query.lower() in nickname.lower():
		handler.add_new_item(title=nickname, subtitle="%s %s@%s:%s" % (protocol.upper(), username, hostname, port), icon="term.png", arg=nickname)
